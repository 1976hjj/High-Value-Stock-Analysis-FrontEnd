...
function ResourceHeader({ industry, stockCode, quote, analysis, valuationDate }: { industry: IndustryConfig; stockCode: string; quote: LiveQuote | null; analysis: IndustryAnalysisResult | null; valuationDate: string }) {
  ...
  return <section className="resource-hero">
    <div><span className="eyebrow">RESOURCE CYCLE CONTROL ROOM · {stock.code}</span><h1><StockLogo stockCode={stock.code} stockName={stock.name} className="stock-logo-hero" />{stock.name}</h1><p>{profile.commodity} · {profile.structure}</p><footer><span>{profile.hedgeRole}</span><span>周期敏感度 {profile.cycleSensitivity.toFixed(2)}</span><span>{profile.costMoat}</span></footer></div>
